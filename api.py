...
from inference_webui import inference
...
def handle(refer_wav_path, prompt_text, prompt_language, text, text_language,sweight,gweight):
    if (
    ...
    if gweight is not None and sweight is not None:
...
def handle_new(text, text_lang, ref_audio_path, prompt_text, prompt_lang,text_split_method="按标点符号切"):
    ...
    with torch.no_grad():
        gen = inference(text, text_lang, 
              ...
              text_split_method=text_split_method, batch_size=20, 
              speed_factor=1.0, ref_text_free=False,
...
async def tts_endpoint(
        text, text_lang, ref_audio_path, prompt_text, prompt_lang,text_split_method
):
    return handle_new(text, text_lang, ref_audio_path, prompt_text, prompt_lang,text_split_method)
...
async def tts_to_audio(request: Request):
    ...
    json_post_raw = await request.json()
    return handle_new(json_post_raw.get("text"),"中英混合", global_config.llama_audio,global_config.llama_text,global_config.llama_lang,global_config.text_split_method)
